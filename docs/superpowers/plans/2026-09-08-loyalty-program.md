...
2. **The `fee_lines` negative-discount mechanism (Task 3) has not been tested against a real order.** This was flagged as a real unknown in the spec — the code is written per the documented technique, but needs a real order placed with real points redeemed to confirm WooCommerce actually reduces the total as expected, with the documented fallback ready if it doesn't.
...
- **Scope:** main Artace cart checkout only. Custom Portraits deposits and Samora are explicitly excluded — the plan includes a step that actively excludes Custom Portraits orders from crediting, not just an assumption that it won't matter.
